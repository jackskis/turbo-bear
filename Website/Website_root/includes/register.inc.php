...
//Reference: $mysqli = mysqli_connect("users.cejsrq6wzm2i.us-west-2.rds.amazonaws.com:3306","jackskis","tuckerdog","UserList");
...
if (isset($_POST['username'], $_POST['email'], $_POST['p']))
{
    ...
    // Defines that the account is not active (until the email link clicked)
    $active = 0;
   ...
   // check existing email  
    ...
    if (empty($error_msg))
    {
        ...
        if ($insert_stmt = $mysqli->prepare("INSERT INTO Members (usersFirstName, usersLastName, studentId, username, email, password, salt, type, active, hash) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"))
        {
            $insert_stmt->bind_param('ssssssssss',$UsersFirstName, $UsersLastName, $studentId, $username, $email, $password, $random_salt, $type, $active, $hash);
            ...
            Username: '.$name.'
            Use the replacement password entered into the site!
